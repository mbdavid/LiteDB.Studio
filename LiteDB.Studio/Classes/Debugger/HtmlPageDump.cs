...
namespace LiteDB.Studio.Classes.Debugger
{
    public class HtmlPageDump
    {
        ...
        private void SpanCaptionItems()
        {
            ...
            if (_pageType == 1)
                SpanHeaderPage();
            else if (_pageType == 2) SpanCollectionPage();
        }
        ...
        private void SpanSegments()
        {
                ...
                for (var i = 0; i <= highestIndex; i++)
                {
                    ...
                    if (position != 0)
                    {
                        ...
                        else
                        {
                            ...
                            if (_buffer[p] == 6 || _buffer[p] == 9) SpanItem<byte>(++p, 0, null, "Len", null);

                            //TODO key??
                        }

                        for (var j = position; j < position + length; j++) _items[j].Color = colorIndex - 1;
                    }
                }
            }
            ...
            for (var i = 0; i < _buffer.Length; i++)
                if (_items[i].Color != -1)
                {
                    if (_items[i].Color != current) color++;
        ...
        private void SpanHeaderPage()
        {
            ...
            while (p < collectionPosition + 4 + BitConverter.ToInt32(_buffer, collectionPosition) - 5)
            {
                ...
                for (var k = initial; k <= p; k++) _items[k].Color = color % _colors.Length;
        ...
        private void SpanCollectionPage()
        {
            ...
            for (var i = 0; i < 5; i++) SpanPageID(32 + i * 4, "DataPageList #" + i, false, true);
            ...
            for (var i = 0; i < indexes; i++)
            {
                ...
                for (var k = initial; k <= h; k++) _items[k].Color = color % _colors.Length;
        ...
        private void RenderHeader()
        {
            ...
            _writer.AppendLine(
                $".rules > div {{ padding: 9px 0 0; height: 31px; width: {BLOCK_WIDTH}px; color: gray; background-color: #f1f1f1; margin: 1px; text-align: center; position: relative; }}");
            _writer.AppendLine(".line { min-width: 1024px; }");
            _writer.AppendLine(
                $".line > a {{ background-color: #d1d1d1; margin: 1px; min-width: {BLOCK_WIDTH}px; height: 30px; display: inline-block; text-align: center; padding: 10px 0 0; position: relative; }}");
            _writer.AppendLine(".line:first-child > a { background-color: #a1a1a1; }");
            _writer.AppendLine(".line > a[href] { color: blue; }");
            _writer.AppendLine(
                ".line > a:before { background-color: white; font-size: 7px; top: -1; left: 0; color: black; position: absolute; content: attr(st); }");
        ...
        private void RenderInfo()
        {
            if (_page.ContainsKey("pageID"))
            {
                ...
                _writer.AppendLine(
                    $"Origin: [{_page["_origin"].AsString}] - Position: {_page["_position"].AsInt64} - Free bytes: {_page["freeBytes"]}");
        ...
        private void RenderBlocks()
        {
            ...
            for (var i = 0; i < _items.Count; i++)
            {
                ...
                var renderText = RenderItem(item, span);
        ...
        private bool RenderItem(PageItem item, int span)
        {
            ...
            if (item.Span > 0)
            {
                ...
                if (s < item.Span && Math.Abs(span) > s) renderText = false;
        ...
        private int SpanPageID(int index, string caption, bool pageAddress, bool pageList)
        {
            ...
            if (pageAddress)
            {
                ...
                _items[index + 4].Text =
                    _items[index + 4].Value == byte.MaxValue ? "-" : _items[index + 4].Value.ToString();
            }

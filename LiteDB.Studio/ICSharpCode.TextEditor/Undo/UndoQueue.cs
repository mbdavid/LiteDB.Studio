...
namespace LiteDB.Studio.ICSharpCode.TextEditor.Undo
{
    ...
    internal sealed class UndoQueue : IUndoableOperation
    {
        ...
        public UndoQueue(Stack<IUndoableOperation> stack, int numops)
        {
            ...
            if (numops > stack.Count) numops = stack.Count;

            for (var i = 0; i < numops; ++i) undolist.Add(stack.Pop());
        }

        public void Undo()
        {
            for (var i = 0; i < undolist.Count; ++i) undolist[i].Undo();
        }

        public void Redo()
        {
            for (var i = undolist.Count - 1; i >= 0; --i) undolist[i].Redo();
        }
    }
}
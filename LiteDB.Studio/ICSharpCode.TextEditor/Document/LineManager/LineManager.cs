...
namespace LiteDB.Studio.ICSharpCode.TextEditor.Document.LineManager
{
    internal sealed class LineManager
    {
        ...
        public IHighlightingStrategy HighlightingStrategy
        {
            ...
            set
            {
                if (highlightingStrategy != value)
                {
                    ...
                    if (highlightingStrategy != null) highlightingStrategy.MarkTokens(document);
                }
            }
        }
        ...
        public void Replace(int offset, int length, string text)
        {
            ...
            if (!string.IsNullOrEmpty(text)) InsertInternal(offset, text);
            //			#if DEBUG
...
//			#endif
            ...
            if (TotalNumberOfLines != oldNumberOfLines)
                OnLineCountChanged(new LineCountChangeEventArgs(document, lineStart,
        ...
        private void RemoveInternal(ref DeferredEventList deferredEventList, int offset, int length)
        {
            ...
            startSegment.RemovedLinePart(ref deferredEventList, offset - startSegmentOffset,
        ...
        public void SetContent(string text)
        {
            ...
            if (text != null) Replace(0, 0, text);
        }

        public int GetVisibleLine(int logicalLineNumber)
        {
            if (!document.TextEditorProperties.EnableFolding) return logicalLineNumber;
            ...
            {
                if (fm.StartLine >= logicalLineNumber) break;
                if (fm.StartLine >= foldEnd)
                {
                    visibleLine += fm.StartLine - foldEnd;
                    if (fm.EndLine > logicalLineNumber) return visibleLine;
                    foldEnd = fm.EndLine;
                }
            }
...
//			Debug.Assert(logicalLineNumber >= foldEnd);
        ...
        {
            if (!document.TextEditorProperties.EnableFolding) return visibleLineNumber;
            var v = 0;
            var foldEnd = 0;
            var foldings = document.FoldingManager.GetTopLevelFoldedFoldings();
            foreach (var fm in foldings)
                if (fm.StartLine >= foldEnd)
                {
                    if (v + fm.StartLine - foldEnd >= visibleLineNumber) break;
        ...
        public int GetNextVisibleLineAbove(int lineNumber, int lineCount)
        {
            ...
            if (document.TextEditorProperties.EnableFolding)
                ...
                {
                    ++curLineNumber;
                    while (curLineNumber < TotalNumberOfLines && (curLineNumber >= lineCollection.Count ||
                                                                  !document.FoldingManager.IsLineVisible(curLineNumber))
                    ) ++curLineNumber;
                }
            else
        ...
        public int GetNextVisibleLineBelow(int lineNumber, int lineCount)
        {
            ...
            if (document.TextEditorProperties.EnableFolding)
                ...
                {
                    --curLineNumber;
                    while (curLineNumber >= 0 && !document.FoldingManager.IsLineVisible(curLineNumber)) --curLineNumber;
                }
            else
                curLineNumber -= lineCount;
        ...
        {
            if (LineCountChanged != null) LineCountChanged(this, e);
        }

        private void OnLineLengthChanged(LineLengthChangeEventArgs e)
        {
            if (LineLengthChanged != null) LineLengthChanged(this, e);
        }

        private void OnLineDeleted(LineEventArgs e)
        {
            if (LineDeleted != null) LineDeleted(this, e);
        }

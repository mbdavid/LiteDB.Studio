...
namespace LiteDB.Studio.ICSharpCode.TextEditor.Document
{
    public sealed class TextUtilities
    {
        ...
        public static string LeadingWhiteSpaceToTabs(string line, int tabIndent)
        {
            ...
            if (i < line.Length) sb.Append(line.Substring(i - consecutiveSpaces));
            return sb.ToString();
        }

        public static bool IsLetterDigitOrUnderscore(char c)
        {
            if (!char.IsLetterOrDigit(c)) return c == '_';
            return true;
        }
...
//						break;
                    ...
                    case '.':
                        ...
                        break;
                    case '"':
                        if (offset < initialOffset - 1) return null;
                        return "\"\"";
                    case '\'':
                        if (offset < initialOffset - 1) return null;
                        return "'a'";
                    ...
                    default:
                        ...
                        var start = offset - 1;
                        if (!IsLetterDigitOrUnderscore(document.GetCharAt(start))) goto done;

                        while (start > 0 && IsLetterDigitOrUnderscore(document.GetCharAt(start - 1))) --start;
                        ...
                        if (word.Length > 0 && !IsLetterDigitOrUnderscore(word[0])) goto done;
                        offset = start;
                        break;
                }
        ...
        {
            while (offset < document.TextLength && char.IsWhiteSpace(document.GetCharAt(offset))) ++offset;
            return offset;
        }
        ...
        public static int FindWordEnd(IDocument document, int offset)
        {
            ...
            while (offset < endPos && IsLetterDigitOrUnderscore(document.GetCharAt(offset))) ++offset;
        ...
        public static int FindWordStart(IDocument document, int offset)
        {
            ...
            while (offset > lineOffset && IsLetterDigitOrUnderscore(document.GetCharAt(offset - 1))) --offset;
        ...
        public static int FindNextWordStart(IDocument document, int offset)
        {
            ...
            while (offset < endPos && GetCharacterType(document.GetCharAt(offset)) == t) ++offset;
        ...
        public static int FindPrevWordStart(IDocument document, int offset)
        {
            ...
            if (offset > 0)
            {
                ...
                while (offset > line.Offset && GetCharacterType(document.GetCharAt(offset - 1)) == t) --offset;
        ...
        public static string GetWordAt(IDocument document, int offset)
        {
            ...
            while (startOffset > 0 && IsWordPart(document.GetCharAt(startOffset - 1))) --startOffset;

            while (endOffset < document.TextLength - 1 && IsWordPart(document.GetCharAt(endOffset + 1))) ++endOffset;

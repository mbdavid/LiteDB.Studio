    ...
    public class Tab : AbstractEditAction
    {
        ...
        private void InsertTabs(IDocument document, ISelection selection, int y1, int y2)
        {
            ...
            for (var i = y2; i >= y1; --i)
            {
                ...
                if (i == y2 && i == selection.EndPosition.Y && selection.EndPosition.X == 0) continue;
...
//				document.Replace(line.Offset,line.Length,newLine);
        ...
        private void InsertTabAtCaretPosition(TextArea textArea)
        {
            switch (textArea.Caret.CaretMode)
            {
                ...
                case CaretMode.OverwriteMode:
                    ...
                    if (indentStr.Length > 1) textArea.InsertString(indentStr.Substring(1));
                    break;
            }
            ...
            if (textArea.SelectionManager.HasSomethingSelected)
            {
                foreach (var selection in textArea.SelectionManager.SelectionCollection)
                {
                    ...
                    if (startLine != endLine)
                    {
                        ...
                        textArea.Document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.LinesBetween, startLine,
                            endLine));
                        textArea.EndUpdate();
                    }
                    else
    ...
    public class ShiftTab : AbstractEditAction
    {
        private void RemoveTabs(IDocument document, ISelection selection, int y1, int y2)
        {
            ...
            {
                var line = document.GetLineSegment(i);
                if (i == y2 && line.Offset == selection.EndOffset) continue;
                if (line.Length > 0) /**** TextPad Strategy:
...
//					string temp = document.GetText(line.Offset,line.Length);
                    if (line.Length > 0)
                    {
                        ...
                        else if (document.GetCharAt(line.Offset) == ' ')
                        {
                            ...
                            if (leadingSpaces >= tabIndent) // just remove tabIndent
                                charactersToRemove = tabIndent;
                            else if (line.Length > leadingSpaces &&
                                     document.GetCharAt(line.Offset + leadingSpaces) == '\t'
                                ) // remove the leading spaces and the following tab as they add up
                                // to just one tab stop
                                charactersToRemove = leadingSpaces + 1;
                            else // just remove the leading spaces
                                charactersToRemove = leadingSpaces;
                        }

                        if (charactersToRemove > 0) document.Remove(line.Offset, charactersToRemove);
                    }
            }
    ...
    public class ToggleLineComment : AbstractEditAction
    {
        ...
        private bool ShouldComment(IDocument document, string comment, ISelection selection, int startLine, int endLine)
        {
            for (var i = endLine; i >= startLine; --i)
            {
                ...
                var lineText = document.GetText(line.Offset, line.Length);
                if (!lineText.Trim().StartsWith(comment)) return true;
            }
        ...
        public override void Execute(TextArea textArea)
        {
            ...
            if (textArea.SelectionManager.HasSomethingSelected)
            {
                ...
                foreach (var selection in textArea.SelectionManager.SelectionCollection)
                    if (!ShouldComment(textArea.Document, comment, selection, selection.StartPosition.Y,
                ...
                foreach (var selection in textArea.SelectionManager.SelectionCollection)
                {
                    textArea.BeginUpdate();
                    if (shouldComment)
                        SetCommentAt(textArea.Document, comment, selection, selection.StartPosition.Y,
                            selection.EndPosition.Y);
                    else
                        RemoveCommentAt(textArea.Document, comment, selection, selection.StartPosition.Y,
                            selection.EndPosition.Y);
                    textArea.Document.UpdateQueue.Clear();
                    textArea.Document.RequestUpdate(new TextAreaUpdate(TextAreaUpdateType.LinesBetween, firstLine,
                        lastLine));
                    textArea.EndUpdate();
                }
    ...
    public class ToggleBlockComment : AbstractEditAction
    {
        ...
        public override void Execute(TextArea textArea)
        {
            ...
            if (commentStart == null || commentStart.Length == 0 || commentEnd == null ||
                commentEnd.Length == 0) return;
            ...
            if (textArea.SelectionManager.HasSomethingSelected)
            {
                ...
                selectionEndOffset = textArea.SelectionManager
                    .SelectionCollection[textArea.SelectionManager.SelectionCollection.Count - 1].EndOffset;
            }
            else
            {
        ...
        public static BlockCommentRegion FindSelectedCommentRegion(IDocument document, string commentStart,
            string commentEnd, int selectionStartOffset, int selectionEndOffset)
        {
            ...
            if (commentStartOffset >= 0) commentStartOffset += selectionStartOffset;

            // Find end of comment in selected text.

            if (commentStartOffset >= 0)
                commentEndOffset = selectedText.IndexOf(commentEnd,
                    commentStartOffset + commentStart.Length - selectionStartOffset);
            else
                commentEndOffset = selectedText.IndexOf(commentEnd);

            if (commentEndOffset >= 0) commentEndOffset += selectionStartOffset;
            ...
            if (commentStartOffset == -1)
            {
                ...
                if (offset > document.TextLength) offset = document.TextLength;
                ...
                if (commentStartOffset >= 0)
                {
                    ...
                    if (commentEndBeforeStartOffset > commentStartOffset) commentStartOffset = -1;
                }
            }
        ...
        private void SetCommentAt(IDocument document, int offsetStart, int offsetEnd, string commentStart,
            string commentEnd)
        {

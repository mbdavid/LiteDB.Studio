...
namespace LiteDB.Studio.ICSharpCode.TextEditor.Gui
{
    ...
    public class GutterMargin : AbstractMargin, IDisposable
    {
        ...
        static GutterMargin()
        {
            var cursorStream = Assembly.GetCallingAssembly()
                .GetManifestResourceStream("LiteDB.Studio.ICSharpCode.TextEditor.Resources.RightArrow.cur");
        ...
        public override Size Size =>
            new Size(textArea.TextView.WideSpaceWidth
        ...
        public override void HandleMouseDown(Point mousepos, MouseButtons mouseButtons)
        {
            ...
            if (realline >= 0 && realline < textArea.Document.TotalNumberOfLines)
            {
                ...
                if ((Control.ModifierKeys & Keys.Shift) != 0)
                {
                    if (!textArea.SelectionManager.HasSomethingSelected && realline != textArea.Caret.Position.Y)
                    {
                        if (realline >= textArea.Caret.Position.Y)
                        {
                            ...
                            if (realline < textArea.Document.TotalNumberOfLines - 1)
                            {
                                textArea.SelectionManager.SetSelection(new DefaultSelection(textArea.Document,
                                    selectionStartPos, new TextLocation(0, realline + 1)));
                        ...
                        else
                        {
                            ...
                            // whole line selection - start of line to start of next line
                            textArea.SelectionManager.SetSelection(new DefaultSelection(textArea.Document,
                                selectionStartPos, new TextLocation(selectionStartPos.X, selectionStartPos.Y)));
                ...
                else
                {
                    ...
                    {
                        textArea.SelectionManager.SetSelection(new DefaultSelection(textArea.Document,
                            selectionStartPos, new TextLocation(selectionStartPos.X, selectionStartPos.Y + 1)));
                    ...
                    else
                    {
                        textArea.SelectionManager.SetSelection(new DefaultSelection(textArea.Document,
                            new TextLocation(0, realline),
                            new TextLocation(textArea.Document.GetLineSegment(realline).Length + 1,
                                selectionStartPos.Y)));
                        textArea.Caret.Position =
                            new TextLocation(textArea.Document.GetLineSegment(realline).Length + 1,
                                selectionStartPos.Y);

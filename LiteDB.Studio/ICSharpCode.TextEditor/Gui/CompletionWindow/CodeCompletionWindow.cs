...
namespace LiteDB.Studio.ICSharpCode.TextEditor.Gui.CompletionWindow
{
    public class CodeCompletionWindow : AbstractCompletionWindow
    {
            ...
            parentForm, control)
        {
            ...
            if (declarationViewWindow == null) declarationViewWindow = new DeclarationViewWindow(parentForm);
        ...
        public static CodeCompletionWindow ShowCompletionWindow(Form parent, TextEditorControl control, string fileName,
            ICompletionDataProvider completionDataProvider, char firstChar, bool showDeclarationWindow,
            bool fixedListViewWidth)
        {
            var completionData =
                completionDataProvider.GenerateCompletionData(fileName, control.ActiveTextAreaControl.TextArea,
                    firstChar);
            if (completionData == null || completionData.Length == 0) return null;
            var codeCompletionWindow = new CodeCompletionWindow(completionDataProvider, completionData, parent, control,
        ...
        private Size GetListViewSize()
        {
            ...
            if (!fixedListViewWidth) width = GetListViewWidth(width, height);
        ...
        private int GetListViewWidth(int defaultWidth, int height)
        {
            ...
            if (totalItemsHeight > height) width += ScrollbarWidth; // Compensate for scroll bar.
            return (int) width;
        }
    }
}
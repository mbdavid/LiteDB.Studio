...
namespace LiteDB.Studio.ICSharpCode.TextEditor.Gui
{
    ...
    public class HRuler : Control
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            var g = e.Graphics;
            var num = 0;
            for (float x = textArea.TextView.DrawingPosition.Left;
                x < textArea.TextView.DrawingPosition.Right;
                x += textArea.TextView.WideSpaceWidth)
            {
                var offset = Height * 2 / 3;
                if (num % 5 == 0) offset = Height * 4 / 5;

                if (num % 10 == 0) offset = 1;
                ++num;

...
namespace LiteDB.Studio.ICSharpCode.TextEditor.Gui
{
    ...
    public class DrawableLine
    {
        ...
        public int LineLength
        {
            ...
            {
                var length = 0;
                foreach (var word in words) length += word.Word.Length;
                return length;
            }
        }
        ...
        public void SetBold(int startIndex, int endIndex, bool bold)
        {
            ...
            for (var i = 0; i < words.Count; i++)
            {
                ...
                else if (startIndex <= pos)
                {
                    ...
                    var newWord = new SimpleTextWord(word.Type, word.Word.Substring(inRegionLength), word.Bold,
                        word.Color);
        ...
        {
            if (word == null || word.Length == 0) return 0f;
        ...
        {
            if (spaceSize.IsEmpty) spaceSize = g.MeasureString("-", boldMonospacedFont, new PointF(0, 0), sf);
            return spaceSize;
        }
                ...
                switch (word.Type)
                {
                    ...
                    case TextWordType.Word:
                        if (word.Word != null && word.Word.Length > 0)
                            xPos += g.MeasureString(word.Word, word.Bold ? boldMonospacedFont : monospacedFont, 32768,
                                sf).Width;
                        break;
                }
        ...
        {
            internal static readonly SimpleTextWord Space =
                new SimpleTextWord(TextWordType.Space, " ", false, Color.Black);
